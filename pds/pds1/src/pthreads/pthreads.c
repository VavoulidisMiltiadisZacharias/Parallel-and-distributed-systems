...
//Struct that contains (as variables) the arguments of triangles_counting
struct args
{
	//int triangles;
	int *row;
...
void coo2csc(
  ...
  for (uint32_t i = 0, last = 0; i < n; i++) {
	...
	*/
	void* triangles_counting(void* argument)
	{
		struct args *arg = (struct args*)argument;
		
		//int triangles = (int)arg->triangles;
    	int *row = (int*)arg->row;
    	int *col = (int*)arg->col;
    	int N = (int)arg->N;
    	int start = (int)arg->start;
...
int main(int argc, char *argv[])
{
    ...
    //Declare some threads
    int not = 12 ; //number of threads
    pthread_t th[not];
    
    //Mutex
    pthread_mutex_init(&mutex, NULL);
    
    //Allocate memory for some structs;
    struct args *arg = (struct args*)malloc(not*sizeof(struct args));
    ...
    for(int  i = 0 ; i < not ; i++)
    {
    	...
    	//Just a print message to check that everything works properly
    	//printf("i:%d  N:%d\n", arg[i].start, arg[i].N);
    	
    	pthread_create(&th[i], NULL, &triangles_counting, (void*)&arg[i]);    	
    }
