	...
	*/
	int triangles_counting(int *row, int* col, int N)
	{
		...
		#pragma omp parallel
		{
			...
			{
				if(i + p < N) //execute if true
				{
					int c = col[i+1+p] - col[i+p];
					for(int j = 1 ; j < c ; j++)
					{
						int x = row[j-1+col[i+p]];
						for(int m = 0 ; m < c-j ; m++)
						{
							int a = col[row[col[i+p]+j+m]]; //start
							int b = col[row[col[i+p]+j+m]+1]; //end
							//printf("p:%d  i:%d  c:%d  j:%d  m:%d  x:%d  a:%d  b:%d", p,i,c,j,m,x,a,b);
							for(int k = a ; k < b ; k++)
							{
								if(row[k] == x)
								{
