...
*
*/ 
void updateLattice(int *evenLattice, int *oddLattice, int n, int k, int limit)
{
	while(k < limit)
    {
    	printf("\n k: %d\n\n", k+1);
...
int main()
{
	...
	int *evenLattice; //Read only at even rounds (nxn size). This is the initial lattice too for this example
	int *oddLattice; //Read only at odd rounds (nxn size)
	...
	evenLattice = (int*)malloc(n*n *sizeof(int));
	oddLattice = (int*)malloc(n*n *sizeof(int));
    ...
    updateLattice(evenLattice, oddLattice, n, k, limit);
