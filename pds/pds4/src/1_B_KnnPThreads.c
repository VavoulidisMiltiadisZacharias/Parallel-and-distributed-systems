...
*
*/
void *setInnerSet(void *arg)
{	
int k = *(int*)arg;	//0, 2
	
	if(n%2 ==0)
	{	
		int j = 0; //innerSet counter (different for every thread)
...
int main()
{
	...
	//Allocate memory for inner and outer set
	if(n%2 == 0)
	{
	...
	//Create not/2 threads to fill innerSet array
	for(int i = 0 ; i < not ; i++)
	{
		if(i < not/2) //Inner set
		{
			int* a = malloc(sizeof(int));
			*a = i*(n/not);
			if(pthread_create(&th[i], NULL, &setInnerSet, a) != 0)
			{
		...
		else //Outer set
		{
			int* a = malloc(sizeof(int));
			*a = i*(n/not);
			if(pthread_create(&th[i], NULL, &setOuterSet, a) != 0)
			{
				perror("\nFailed to create thread\n");
			}
			//free(a);
