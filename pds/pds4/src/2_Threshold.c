...
//Global variables
int n = 216; //number of points
int dimensions = 256; //number of dimensions of each point
struct point* points;
int not = 24; //number of threads to create
int times = 0;
...
/*
* Function that calculates the distance of all points from vantage point
*/
void* calculateDistancesParallel(void *arg)
{
	//Local variables
	int k = *(int*)arg; //0, 3, 6
	int j;
	...
	*/
	for(int i = 0 ; i < n/not ; i++)
	{
		for(int j = 0 ; j < points[i].dimensions ; j++)
		{
			//Calculate sum of squares
			points[k+i].distance += (points[n-1].coordinates[j]-points[k+i].coordinates[j])*(points[n-1].coordinates[j]-points[k+i].coordinates[j]);
		}
		times++;
		//printf("\n%d point is ok\n", i);
	}
	...
	*/
	for(int i = k*(n/not) ; i < k*(n/not)+(n/not) ; i++)
	{
...
/*
* Function that runs the parallel veriosn of the algorithm
*/
void runParallelVersion()
{
	//Declare not threads
	pthread_t th[not];
	...
	for(int i = 0 ; i < not ; i++)
	{
		...
		*a = i*(n/not);
		if(pthread_create(&th[i], NULL, &calculateDistancesParallel, a) != 0)
		{
...
int main()
{
	...
	struct point *medianPoint; // return of kthSmallest
	...
	//int not; //not stands for "numberOfThreads" to create
	...
	// Check and choose which version to run
	if(n > 95) //Run parallel version
	{
		printf("\n\n");
		printf("RUNNING PARALLEL VERSION \n\n");
		runParallelVersion();
	}
	else //Run sequanteial version
	{
		printf("\n\n");
	...
	//Select median using "quickselect" algorithm
	if(n%2 == 0)
	{
		k_th = n/2;
		medianPoint = kthSmallest(points, 0, n-1, k_th);
		median = medianPoint->distance;
